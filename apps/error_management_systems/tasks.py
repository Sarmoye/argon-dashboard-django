...
def process_cis_error_report(latest_file=None):
    """
    1) Lit le CSV CIS du jour (ou celui spécifié)
    2) Obtient un token auprès de l'API interne
    3) Envoie en batch les événements vers create_event_api
    
    Cette fonction est maintenant appelée directement depuis task_execute_cis_error_report
    """
    ...
    # Si aucun fichier n'est spécifié, trouver le dernier fichier du jour
    if not latest_file:
        ...
        if resp.status_code < 400:  # Si la réponse est OK
            try:
                ...
                if "access" in token_data:
                    ...
                    return {"status": "error", "msg": "no_token", "detail": resp.text}
            except Exception as json_exc:
                logger.error(f"Impossible de parser la réponse JSON: {str(json_exc)}")
                return {"status": "error", "msg": "json_parse_error", "detail": str(json_exc)}
        else:
            logger.error(f"Erreur d'authentification: {resp.status_code} {resp.reason}: {resp.text}")
            return {"status": "error", "msg": "auth_error", "detail": f"{resp.status_code}: {resp.text}"}
    except Exception as exc:
        logger.exception(f"Exception lors de l'authentification: {str(exc)}")
        return {"status": "error", "msg": "auth_exception", "detail": str(exc)}
    ...
    try:
        ...
        return {"status": "error", "msg": "api_error", "detail": str(exc)}
...
def task_execute_cis_error_report(self):
    ...
    Appelle ensuite automatiquement la fonction process_cis_error_report 
    ...
    try:
        ...
        # --- 5. Après succès de la génération, traiter et envoyer le rapport
        logger.info("Lancement du traitement du rapport pour envoi à l'API...")
        processing_result = process_cis_error_report(output_file)
        
        if processing_result.get("status") != "success":
            logger.error("Échec du traitement du rapport: %s", processing_result)
            return {
                "status": "error",
                "phase": "processing",
        ...
        # --- 6. Si tout s'est bien passé, renvoyer succès
        logger.info("Rapport CIS généré et envoyé avec succès")
        return {
            "status": "success",
